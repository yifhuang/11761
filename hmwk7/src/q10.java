...
public class q10 {
	void solve() throws IOException{
		String trainData = Utils.filteInFile("hmm-decode.txt");
		int loops = 10;
		int numS = 2;
		int numSy =27;
		...
		String path = Utils.naive(logll,factorsA,numOfFactorA, factor, trainData.toCharArray(), lamdas, bs, intial, end, numSy, numS, numT);
		//Double betas[][] = Utils.alphaGen(factorsA,numOfFactorB, factor, trainData.toCharArray(), lamdas, bs, intial, end, numSy, numS, numT);
		System.out.println(path);
		System.out.println(numOfFactorA[0] +" alpha: "+1.0/numT*(Math.log(logll[0])-1.0*numOfFactorA[0]*Math.log(factor))/Math.log(2));
	}
}
